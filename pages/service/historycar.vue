      ...
      <scroll-view scroll-y class="content">
        <view v-for="(item, index) in historyList" :key="index" class="history-item">
          <view class="item-header">
            <text class="item-title">申请车辆车牌号：{{ item.license }}</text>
            <text class="item-date">{{ item.currentTime }}</text>
          </view>
		  <view class="item-header">
		  			<text class="item-title">申请进入时间：{{ item.time }}</text>
		            </view>
  ...
  export default {
    ...
    methods: {
      getdata() {
        ...
        console.log(Value);
        uni.request({
          url: `http://192.168.50.101:8090/auth/getHistoryvehicle?studentid=${this.studentid}`,
          success: (res) => {
            console.log(res);
