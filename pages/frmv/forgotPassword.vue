<template>
	<view class="wrap">
		<u-form :model="model" ref="uForm">
			...
			<u-input :password-icon="true" :border="border" type="password" v-model="model.newpassword" placeholder="请输入密码"></u-input>
           </u-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			codeChange(text) {
						...
						getCode() {
							if(this.$refs.uCode.canGetCode) {
								...
								uni.request({
									url: 'http://192.168.50.101:8090/auth/sendVerficationpassword',
									data: {
			...
			submit() {
				if (this.model.newpassword != this.model.rpassword) {
